		...
		if INJECTED_SCRIPTS[script] then
			...
			if not xmlFile then
				...
				if not chunk then
					...
					local newEnv = { globalVars = {}, wrappers = { commands = {}, events = {}, binds = {}, timers = {} } }
...
local function uninjectScript ( script )
	if exports.global:isPlayerScripter( getLocalPlayer( ) ) and type(script) == "string" and #script > 0 then
		if not INJECTED_SCRIPTS[script] then
			outputChatBox("Script '" .. script .. "' not loaded.", 255, 0, 0)
			return false
		else
			...
			outputChatBox("Script '" .. script .. "' unloaded.", 0, 255, 0)
			return true
		end
	end
