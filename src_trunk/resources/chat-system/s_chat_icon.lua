...
function sendChatIconShown()
	...
	for key, value in ipairs(getElementsByType("player")) do
		...
		if ( getDistanceBetweenPoints3D(px, py, pz, vx, vy, vz) <= 25 ) and not chaticonsHidden[value] then -- only send if they can see it and have chaticons enabled
...
function sendChatIconHidden()
	...
	for key, value in ipairs(getElementsByType("player")) do
		...
		if ( getDistanceBetweenPoints3D(px, py, pz, vx, vy, vz) <= 25 ) and not chaticonsHidden[value] then -- only send if they can see it and have chaticons enabled, persons out of range who COULD see it before, are handled clientside
