...
wBeta, lBetaInfo, bSubmitBeta, bDenyBeta = nil

function hasBeta()	
	local xmlRoot = xmlLoadFile( "beta.xml" )
	if (xmlRoot) then
		local keyNode = xmlFindChild(xmlRoot, "key", 0)
		local keysumNode = xmlFindChild(xmlRoot, "keysum", 0)
		local key, keysum = nil
		
		if (keyNode) then
			key = xmlNodeGetValue(keyNode)
		end
		
		if (keysumNode) then
			keysum = xmlNodeGetValue(keysumNode)
		end
		
		if (key and keysum) then
			if ( keysum == md5(salt .. key .. salt) ) then -- valid key
				return true
			else -- hacked key
				return false
			end
...
function promptBeta()
	...
	if not ( hasBeta() ) then
		...
		wBeta = guiCreateWindow(x, y, width, height, "Beta Eligibility!", false)
		
		lBetaInfo = guiCreateLabel(0.02, 0.15, 0.95, 1.0, "You are eligible for a BETA version of the script.\n\nThis version may be buggy.\n\nIf reporting bugs ensure you mark them as private\n\nThis is TOP SECRET.", true, wBeta)
		guiLabelSetHorizontalAlign(lBetaInfo, "center", true)
		guiSetFont(lBetaInfo, "default-bold-small")

		bSubmitBeta = guiCreateButton(0.15, 0.75, 0.35, 0.15, "Enroll", true, wBeta)
		addEventHandler("onClientGUIClick", bSubmitBeta, submitBeta, false)
		guiSetFont(bSubmitBeta, "default-bold-small")
		guiSetAlpha(bSubmitBeta, 0.5)
		
		bDenyBeta = guiCreateButton(0.5, 0.75, 0.35, 0.15, "Decline", true, wBeta)
		addEventHandler("onClientGUIClick", bDenyBeta, cleanupBeta, false)
		guiSetFont(bDenyBeta, "default-bold-small")
		guiSetAlpha(bDenyBeta, 0.5)
	end
end

function submitBeta()
	-- write a beta file
	local rootNode = xmlCreateFile("beta.xml", "beta")
	
	local keyNode = xmlCreateChild(rootNode, "key")
	local keysumNode = xmlCreateChild(rootNode, "keysum")
	
	local key = md5(salt .. getElementData(getLocalPlayer(), "gameaccountusername"))
	local keysum = md5(salt .. key .. salt)
	xmlNodeSetValue(keyNode, key)
	xmlNodeSetValue(keysumNode, keysum)
...
function drawBG()
	...
	elseif (state >= 2 ) then -- main XMB
		...
		if (currentItemAlpha < 1.0) then
			currentItemAlpha = currentItemAlpha + 0.05
		end
...
function selectItemFromVerticalMenu()
	if ( mainMenuItems[currentItem]["text"] == "Characters" ) then
		...
		if ( mainMenuItems[currentItem]["text"] == "Logout" ) then
			-- cleanup
